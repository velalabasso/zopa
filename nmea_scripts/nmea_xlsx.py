...
def haversine_nm(lat1, lon1, lat2, lon2):
    ...
    return (
        2 * R * np.arctan2(np.sqrt(a), np.sqrt(1 - a))
    ) / 1852.0
...
def parse_nmea(file_path):
    ...
    with open_func(file_path, "rt") as f:
        ...
        for line in f:
            ...
            if msg == "$GPRMC":
                ...
                current["lat_raw"] = nmea_to_decimal(
                    parts[3],
                    parts[4]
                )

                current["lon_raw"] = nmea_to_decimal(
                    parts[5],
                    parts[6]
                )
    ...
    df["allure"] = df["TWA"].apply(compute_allure)
...
for log_dir_name in os.listdir(base_folder):
    ...
    # =====================================================
    # SKIP SI DÉJÀ PROCESSÉ
    # =====================================================

    if os.path.exists(out_xlsx) and os.path.exists(out_csv):

        print("Already processed:", gz_file)
        continue
    ...
    grouped["datetime"] = grouped["datetime"].dt.strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
